...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => genMonths(props.locale as Locale || enUS), []);
  const YEARS = React.useMemo(() => genYears(props.locale as Locale || enUS, yearRange), []);
...
export interface DateTimePickerFormFieldProps<T extends FieldValues> {
...
export function DateTimePickerFormField<T extends FieldValues>({
  ...
  const localeParsed = React.useMemo(() => {
    switch (locale) {
      case 'en':
        return enUS
      case 'es':
